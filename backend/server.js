...
app.post('/api/complaints', (req, res) => {
    const { apartment_no, complaint_type, complaint_text } = req.body;

    // Validate data
    if (!apartment_no || !complaint_type || !complaint_text) {
        return res.status(400).json({ success: false, message: 'Missing required fields' });
    }

    // Insert into database
    const query = `
        INSERT INTO complaints (apartment, complaint_type, complaint_text)
        VALUES (?, ?, ?)
    `;
    db.query(query, [apartment_no, complaint_type, complaint_text], (err, result) => {
        if (err) {
            console.error('Database error:', err);
            console.error('SQL Query:', query); // Log the query
